...
//package com.sakuravillagesimple;
//
...
//
//
//    //它自带一个 Looper，使得它能够处理 Message 和 RunnableRunnable
//    //通过 getLooper() 方法获取该线程的 Looper
...
//
//    /**
//     * 视频播放本身不应该占用主线程的所有资源，因此我们可以将视频渲染交给一个独立的线程或者硬件加速的方式来处理。常见的方法是使用 SurfaceView 或 TextureView，这些视图可以将视频帧交给 GPU 来渲染，而不是直接由主线程处理。这样可以减轻主线程的压力。
//     *
//     * SurfaceView：一个专门用于视频渲染的 View，能够利用硬件加速来播放视频，避免占用主线程的时间。视频播放的每一帧都由系统在底层进行绘制和显示，主线程只负责触发显示操作，渲染过程是异步的。
//     *
//     * TextureView：与 SurfaceView 类似，也可以用来显示视频，但是它允许在主线程中进行更多的操作，例如可以将视频与其他 UI 组件进行层叠显示，支持更多的动态效果。
//     */
//    //重写onDraw
//    @Override
...
//            }
//            mResumeTryCount++;
//            if (mResumeTryCount > 4 || master.flame.danmaku.ui.widget.DanmakuView.super.isShown()) {
//                drawHandler.resume();
//            } else {
